...
export function generateVettingPDF(opts) {
  return new Promise((resolve, reject) => {
    const { carrierData: d, verdict, tier, reasons, authorityDays, override, generatedAt, recordId } = opts;
    ...
    const doc = new PDFDocument({
      ...
      margin: 50,
      info: {
        Title:    'Carrier Vetting Audit Certificate — Precision Transport',
        Author:   'Precision Transport Vetting System',
        Creator:  'Precision Transport Vetting System',
        Producer: 'pdfkit',
      },
    });
    ...
    doc.fontSize(9).font('Helvetica').fillColor('#a8c4e0')
       .text('Precision Transport  ·  Internal Use Only', L, 72, { width: W, align: 'center' });
    ...
    basicRow(doc, L, W, 'Unsafe Driving BASIC',    d.unsafeDrivingBasic,    55);
    basicRow(doc, L, W, 'Crash Indicator BASIC',   d.crashIndicatorBasic,   55);
    basicRow(doc, L, W, 'Hours of Service BASIC',  d.hosBasic,              55);
    basicRow(doc, L, W, 'Vehicle Maintenance BASIC', d.vehicleMaintenanceBasic, 55);
    ...
    oosRow(doc, L, W, 'Truck OOS %',  d.truckOosPct,  d.nationalAvgTruckOos  || 5.5);
    oosRow(doc, L, W, 'Driver OOS %', d.driverOosPct, d.nationalAvgDriverOos || 5.2);
    ...
    doc.fontSize(7.5).font('Helvetica').fillColor(C.gray)
       .text(
         'This document was generated by the Precision Transport Carrier Vetting System and reflects whether ' +
         'this carrier meets Precision Transport\u2019s internal vetting criteria at the time of generation. ' +
...
function metaRow(doc, L, W, label1, val1, label2, val2) {
  // Inline chaining: "Label1: value1     Label2: value2" on one line.
  // Using continued: true avoids any calculated-Y fragility.
...
function dataRow(doc, L, W, label, value) {
  ...
  doc.fontSize(9).font('Helvetica-Bold').fillColor(C.darkgray)
     .text(`${label}:`, L, y, { width: 180 });
  doc.font('Helvetica').fillColor(C.black)
     .text(String(value ?? '—'), L + 182, y, { width: W - 182 });
  doc.moveDown(0.15);
}
...
function flagRow(doc, L, W, label, val) {
  ...
  doc.font('Helvetica-Bold').fillColor(on ? C.red : C.black)
     .text(on ? 'YES' : 'No', L + 182, y, { width: W - 182 });
  doc.moveDown(0.15);
}
...
function basicRow(doc, L, W, label, rawVal, threshold) {
  ...
  const display = isNA ? 'N/A' : `${val.toFixed(2)}%${over ? `  \u26a0 ABOVE ${threshold}% THRESHOLD` : ''}`;
  const y = doc.y;
  doc.fontSize(9).font('Helvetica-Bold').fillColor(C.darkgray)
     .text(`${label}:`, L, y, { width: 180 });
  doc.font(over ? 'Helvetica-Bold' : 'Helvetica').fillColor(over ? C.red : C.black)
     .text(display, L + 182, y, { width: W - 182 });
  doc.moveDown(0.15);
}

function oosRow(doc, L, W, label, rawVal, nationalAvg) {
  const val  = parseFloat(rawVal);
  const isNA = isNaN(val);
  const over = !isNA && val >= parseFloat(nationalAvg) * 2;
  const display = isNA ? 'N/A' : `${val.toFixed(2)}%${over ? '  \u26a0 ABOVE DOUBLE NATIONAL AVG' : ''}`;
  const y = doc.y;
  doc.fontSize(9).font('Helvetica-Bold').fillColor(C.darkgray)
     .text(`${label}:`, L, y, { width: 180 });
